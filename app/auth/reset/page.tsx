...
export default function ResetPasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    else setMessage("Password updated. You can continue to PostMaker.");
  };
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-[#f7f8fc] p-5">
      <div className="w-full max-w-md rounded-[28px] border bg-white p-7 shadow-xl">
        <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500 to-violet-600 font-black text-white">P</div>
        <h1 className="mt-5 text-2xl font-black text-slate-950">Set a new password</h1>
        <p className="mt-2 text-sm text-slate-500">Enter a new password for your PostMaker account.</p>
        ...
        <Link href="/" className="mt-5 inline-flex text-xs font-semibold text-violet-600">Back to PostMaker</Link>
      </div>
    </main>
  );
}
